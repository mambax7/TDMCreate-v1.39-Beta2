 ...
 */
define('{$mod_request_notrim}', 1);
define('{$mod_request_allowraw}', 2);
define('{$mod_request_allowhtml}', 4);

/**
 * {$mod_name_request} Class
 * This class serves to provide a common interface to access
 * request variables.  This includes \$_POST, \$_GET, and naturally \$_REQUEST.  Variables
 * can be passed through an input filter to avoid injection or returned raw.
 */
class {$mod_name_request}
{
    ...
    static function getVar(\$name, \$default = null, \$hash = 'default', \$type = 'none', \$mask = 0)
    {
        ...
        if (isset(\$input[\$name]) && \$input[\$name] !== null) {
            ...
            \$var = {$mod_name_request}::_cleanVar(\$input[\$name], \$mask, \$type);
            // Handle magic quotes compatability
            if (get_magic_quotes_gpc() && (\$var != \$default) && (\$hash != 'FILES')) {
                \$var = {$mod_name_request}::_stripSlashesRecursive(\$var);
            }
        } else if (\$default !== null) {
            // Clean the default value
            \$var = {$mod_name_request}::_cleanVar(\$default, \$mask, \$type);
        } else {
            \$var = \$default;
        }
        return \$var;
    ...
    {
        return {$mod_name_request}::getVar(\$name, \$default, \$hash, 'int');
    }
    ...
    {
        return {$mod_name_request}::getVar(\$name, \$default, \$hash, 'float');
    }
    ...
    {
        return {$mod_name_request}::getVar(\$name, \$default, \$hash, 'bool');
    }
    ...
    {
        return {$mod_name_request}::getVar(\$name, \$default, \$hash, 'word');
    }
    ...
    {
        return {$mod_name_request}::getVar(\$name, \$default, \$hash, 'cmd');
    }
    ...
    static function getString(\$name, \$default = '', \$hash = 'default', \$mask = 0)
    {
        ...
        return (string){$mod_name_request}::getVar(\$name, \$default, \$hash, 'string', \$mask);
    }

    static function getArray(\$name, \$default = array(), \$hash = 'default')
    {
        return {$mod_name_request}::getVar(\$name, \$default, \$hash, 'array');
    }

    static function getText(\$name, \$default = '', \$hash = 'default')
    {
        return (string){$mod_name_request}::getVar(\$name, \$default, \$hash, 'string', PUBLISHER_REQUEST_ALLOWRAW);
    }
    ...
    static function get(\$hash = 'default', \$mask = 0)
    {
        ...
        \$result = {$mod_name_request}::_cleanVar(\$input, \$mask);
        // Handle magic quotes compatability
        if (get_magic_quotes_gpc() && (\$hash != 'FILES')) {
            \$result = {$mod_name_request}::_stripSlashesRecursive(\$result);
        }
        return \$result;
    }
    ...
    static function set(\$array, \$hash = 'default', \$overwrite = true)
    {
        foreach (\$array as \$key => \$value) {
            {$mod_name_request}::setVar(\$key, \$value, \$hash, \$overwrite);
        }
    }
    ...
    {
        {$mod_name_request}::_cleanArray(\$_FILES);
        {$mod_name_request}::_cleanArray(\$_ENV);
        {$mod_name_request}::_cleanArray(\$_GET);
        {$mod_name_request}::_cleanArray(\$_POST);
        {$mod_name_request}::_cleanArray(\$_COOKIE);
        {$mod_name_request}::_cleanArray(\$_SERVER);
        if (isset(\$_SESSION)) {
            {$mod_name_request}::_cleanArray(\$_SESSION);
        }
        \$REQUEST = \$_REQUEST;
        \$GET = \$_GET;
        \$POST = \$_POST;
    ...
    static function _cleanVar(\$var, \$mask = 0, \$type = null)
    {
        ...
        if (\$mask & 2) {
            ...
            if (is_null(\$safeHtmlFilter)) {
                \$safeHtmlFilter = {$mod_name_filter_input}::getInstance(null, null, 1, 1);
            }
            \$var = \$safeHtmlFilter->clean(\$var, \$type);
        } else {
            // Since no allow flags were set, we will apply the most strict filter to the variable
            if (is_null(\$noHtmlFilter)) {
                \$noHtmlFilter = {$mod_name_filter_input}::getInstance( /* \$tags, \$attr, \$tag_method, \$attr_method, \$xss_auto */);
            }
    ...
    {
        \$value = is_array(\$value) ? array_map(array('{$mod_name_request}', '_stripSlashesRecursive'), \$value) : stripslashes(\$value);
        return \$value;
    }
}

/**
 * {$mod_name_filter_input} is a class for filtering input from any data source
 * Forked from the php input filter library by: Daniel Morris <dan@rootcube.com>
 * Original Contributors: Gianpaolo Racca, Ghislain Picard, Marco Wandschneider, Chris Tobin and Andrew Eddie.
 *
 * @author      Louis Landry <louis.landry@joomla.org>
 */
class {$mod_name_filter_input}
{
     ...
     *      <pre>  \$filter = & {$mod_name_filter_input}::getInstance();</pre>
     *
     * @static
     *
     * @param   array   \$tagsArray  list of user-defined tags
     ...
     * @param   int     \$xssAuto    Only auto clean essentials = 0, Allow clean blacklisted tags/attr = 1
     *
     * @return  object  The {$mod_name_filter_input} object.
     * @since   1.5
     */
    public function & getInstance(\$tagsArray = array(), \$attrArray = array(), \$tagsMethod = 0, \$attrMethod = 0, \$xssAuto = 1)
    {
        ...
        if (empty (\$instances[\$sig])) {
            \$instances[\$sig] = new {$mod_name_filter_input}(\$tagsArray, \$attrArray, \$tagsMethod, \$attrMethod, \$xssAuto);
        }
    ...
    public function clean(\$source, \$type = 'string')
    {
        ...
        switch (strtoupper(\$type)) {
            ...
            case 'STRING' :
                ...
                if (isset(\$this) && is_a(\$this, '{$mod_name_filter_input}')) {
                    \$filter =& \$this;
                } else {
                    \$filter = {$mod_name_filter_input}::getInstance();
                }
            ...
            default :
                // Check for static usage and assign \$filter the proper variable
                if (is_object(\$this) && get_class(\$this) == '{$mod_name_filter_input}') {
                    \$filter =& \$this;
                } else {
                    \$filter = {$mod_name_filter_input}::getInstance();
                }
    ...
    protected function _cleanAttributes(\$attrSet)
    {
        ...
        for (\$i = 0; \$i < count(\$attrSet); \$i++) {
            ...
            if ({$mod_name_filter_input}::checkAttribute(\$attrSubSet)) {
                continue;
            }
